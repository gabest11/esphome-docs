...
.. note::
    ...
    ``buffer_size`` on ESP8266 is actually four times as big, the given number is used to allocate a 32-bit array for microsecond timestamps. ``buffer_size: 256`` will allocate 1kB. The default setting will actually allocate 4kB, a significant portion of the 80kB total memory.
...
Alternatively (this the new prefered way):
...
In this case, you can just use ``remote_transmitter.transmit_rc_switch_raw`` or any other device specific ``trasmit_*`` action.
